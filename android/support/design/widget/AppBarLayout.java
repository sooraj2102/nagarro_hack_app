...
public class AppBarLayout extends LinearLayout
{
  ...
  protected int[] onCreateDrawableState(int paramInt)
  {
    ...
    label84: for (int j = R.attr.state_collapsed; ; j = -R.attr.state_collapsed)
    {
      arrayOfInt1[1] = j;
  ...
  public static class Behavior extends HeaderBehavior<AppBarLayout>
  {
    ...
    private void animateOffsetWithDuration(CoordinatorLayout paramCoordinatorLayout, AppBarLayout paramAppBarLayout, int paramInt1, int paramInt2)
    {
      ...
      if (this.mOffsetAnimator == null)
      {
        ...
        this.mOffsetAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(paramCoordinatorLayout, paramAppBarLayout)
        {
          public void onAnimationUpdate(ValueAnimator paramValueAnimator)
          {
            AppBarLayout.Behavior.this.setHeaderTopBottomOffset(this.val$coordinatorLayout, this.val$child, ((Integer)paramValueAnimator.getAnimatedValue()).intValue());
          }
        });
      }
      while (true)
      ...
      while (true)
      {
        ...
        if (i < j)
        {
          ...
          if (!(localBehavior instanceof AppBarLayout.ScrollingViewBehavior))
            break label81;
          int m = ((AppBarLayout.ScrollingViewBehavior)localBehavior).getOverlayTop();
          k = 0;
          if (m != 0)
            k = 1;
        }
    ...
    private void snapToChildIfNeeded(CoordinatorLayout paramCoordinatorLayout, AppBarLayout paramAppBarLayout)
    {
      ...
      label138: label177: for (int i2 = n; ; i2 = m)
      {
    ...
    boolean canDragView(AppBarLayout paramAppBarLayout)
    {
      ...
      while ((localView != null) && (localView.isShown()) && (!localView.canScrollVertically(-1)));
      return false;
    }
    ...
    public boolean onLayoutChild(CoordinatorLayout paramCoordinatorLayout, AppBarLayout paramAppBarLayout, int paramInt)
    {
      ...
      while (true)
      {
        ...
        label203: setHeaderTopBottomOffset(paramCoordinatorLayout, paramAppBarLayout, k);
        continue;
        label215: if ((i & 0x1) == 0)
          continue;
        if (j != 0)
    ...
    public void onNestedPreScroll(CoordinatorLayout paramCoordinatorLayout, AppBarLayout paramAppBarLayout, View paramView, int paramInt1, int paramInt2, int[] paramArrayOfInt, int paramInt3)
    {
      ...
      for (int j = i + paramAppBarLayout.getDownNestedPreScrollRange(); ; j = 0)
      {
        ...
        return;
        label50: i = -paramAppBarLayout.getUpNestedPreScrollRange();
      }
    }
    ...
    public boolean onStartNestedScroll(CoordinatorLayout paramCoordinatorLayout, AppBarLayout paramAppBarLayout, View paramView1, View paramView2, int paramInt1, int paramInt2)
    {
      ...
      for (int i = 1; ; i = 0)
      {
        if ((i != 0) && (this.mOffsetAnimator != null))
          this.mOffsetAnimator.cancel();
    ...
    int setHeaderTopBottomOffset(CoordinatorLayout paramCoordinatorLayout, AppBarLayout paramAppBarLayout, int paramInt1, int paramInt2, int paramInt3)
    {
      ...
      if ((paramInt2 != 0) && (i >= paramInt2) && (i <= paramInt3))
      {
        ...
        label134: label141: for (int n = -1; ; n = 1)
        {
    ...
    protected static class SavedState extends AbsSavedState
    {
      ...
      {
        public AppBarLayout.Behavior.SavedState createFromParcel(Parcel paramParcel)
        {
          return new AppBarLayout.Behavior.SavedState(paramParcel, null);
        }
