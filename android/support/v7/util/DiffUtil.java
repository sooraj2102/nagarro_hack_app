...
public class DiffUtil
{
  private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator()
  {
    ...
    {
      int i = paramSnake1.x - paramSnake2.x;
      if (i == 0)
    ...
    while (!localArrayList2.isEmpty())
    {
      ...
      if (localSnake != null)
      {
        ...
        if (localArrayList3.isEmpty())
        {
          ...
          label221: int m = localRange1.oldListStart;
          ...
          label285: localArrayList2.add(localRange2);
          if (!localSnake.reverse)
            break label493;
        ...
        while (true)
        {
          ...
          break label221;
          label382: if (localSnake.removal)
          {
          ...
          break label285;
          label456: localRange1.oldListStart = (localSnake.x + localSnake.size);
          localRange1.newListStart = (1 + (localSnake.y + localSnake.size));
          continue;
          label493: localRange1.oldListStart = (localSnake.x + localSnake.size);
          localRange1.newListStart = (localSnake.y + localSnake.size);
        }
      }
  ...
  private static Snake diffPartial(Callback paramCallback, int paramInt1, int paramInt2, int paramInt3, int paramInt4, int[] paramArrayOfInt1, int[] paramArrayOfInt2, int paramInt5)
  {
    ...
    for (int i1 = 0; ; i1++)
    {
      ...
      label386: for (int i3 = -i1; i3 <= i1; i3 += 2)
      {
        int i4 = i3 + k;
        int i5;
    ...
    label669: throw new IllegalStateException("DiffUtil hit an unexpected case while trying to calculate the optimal path. Please make sure your data is not changing during the diff calculation.");
  }
  ...
  public static class DiffResult
  {
    ...
    private boolean findMatchingItem(int paramInt1, int paramInt2, int paramInt3, boolean paramBoolean)
    {
      ...
      for (int m = paramInt3; ; m--)
      {
        ...
        label265: j = localSnake.x;
        k = localSnake.y;
      }
      label285: return false;
    }
    ...
    private static DiffUtil.PostponedUpdate removePostponedUpdate(List<DiffUtil.PostponedUpdate> paramList, int paramInt, boolean paramBoolean)
    {
      ...
      DiffUtil.PostponedUpdate localPostponedUpdate1;
      while (true)
        if (i >= 0)
        {
    ...
    public void dispatchUpdatesTo(ListUpdateCallback paramListUpdateCallback)
    {
      ...
      label247: localBatchingListUpdateCallback.dispatchLastEvent();
    }

    public void dispatchUpdatesTo(RecyclerView.Adapter paramAdapter)
    {
      dispatchUpdatesTo(new ListUpdateCallback(paramAdapter)
      {
        public void onChanged(int paramInt1, int paramInt2, Object paramObject)
        {
          this.val$adapter.notifyItemRangeChanged(paramInt1, paramInt2, paramObject);
        }

        public void onInserted(int paramInt1, int paramInt2)
        {
          this.val$adapter.notifyItemRangeInserted(paramInt1, paramInt2);
        }

        public void onMoved(int paramInt1, int paramInt2)
        {
          this.val$adapter.notifyItemMoved(paramInt1, paramInt2);
        }

        public void onRemoved(int paramInt1, int paramInt2)
        {
          this.val$adapter.notifyItemRangeRemoved(paramInt1, paramInt2);
        }
      });
    }
 ...
 * Qualified Name:     android.support.v7.util.DiffUtil
