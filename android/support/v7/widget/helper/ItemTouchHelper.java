  ...
  implements RecyclerView.OnChildAttachStateChangeListener
{
  ...
  private final RecyclerView.OnItemTouchListener mOnItemTouchListener = new RecyclerView.OnItemTouchListener()
  {
    public boolean onInterceptTouchEvent(RecyclerView paramRecyclerView, MotionEvent paramMotionEvent)
    {
      ...
      if (i == 0)
      {
        ...
        {
          ItemTouchHelper.RecoverAnimation localRecoverAnimation = ItemTouchHelper.this.findAnimation(paramMotionEvent);
    ...
    public void onTouchEvent(RecyclerView paramRecyclerView, MotionEvent paramMotionEvent)
    {
      ...
      do
      {
        ...
        do
        {
          ...
          j = paramMotionEvent.findPointerIndex(ItemTouchHelper.this.mActivePointerId);
          if (j >= 0)
            ItemTouchHelper.this.checkSelectForSwipe(i, paramMotionEvent, j);
      ...
      while (paramMotionEvent.getPointerId(k) != ItemTouchHelper.this.mActivePointerId);
      int m = 0;
      if (k == 0)
        m = 1;
      ItemTouchHelper.this.mActivePointerId = paramMotionEvent.getPointerId(m);
  ...
  private int checkHorizontalSwipe(RecyclerView.ViewHolder paramViewHolder, int paramInt)
  {
    if ((paramInt & 0xC) != 0)
    {
      ...
      label148: for (int j = 8; ; j = 4)
      {
        float f4 = Math.abs(f2);
        if (((j & paramInt) == 0) || (i != j) || (f4 < this.mCallback.getSwipeEscapeVelocity(this.mSwipeEscapeVelocity)) || (f4 <= Math.abs(f3)))
          break label154;
        return j;
        i = 4;
        break;
      }
      label154: float f1 = this.mRecyclerView.getWidth() * this.mCallback.getSwipeThreshold(paramViewHolder);
  ...
  private int checkVerticalSwipe(RecyclerView.ViewHolder paramViewHolder, int paramInt)
  {
    if ((paramInt & 0x3) != 0)
    {
      ...
      label145: for (int j = 2; ; j = 1)
      {
        float f4 = Math.abs(f3);
        if (((j & paramInt) == 0) || (j != i) || (f4 < this.mCallback.getSwipeEscapeVelocity(this.mSwipeEscapeVelocity)) || (f4 <= Math.abs(f2)))
          break label151;
        return j;
        i = 1;
        break;
      }
      label151: float f1 = this.mRecyclerView.getHeight() * this.mCallback.getSwipeThreshold(paramViewHolder);
    ...
    else
    {
      ...
      default:
        ...
        if (i == 2)
        {
          ...
          3 local3 = new RecoverAnimation(localViewHolder2, n, i, f3, f4, f2, f1, m, localViewHolder2)
          {
    ...
    implements Animator.AnimatorListener
  {
    ...
    RecoverAnimation(RecyclerView.ViewHolder paramViewHolder, int paramInt1, int paramInt2, float paramFloat1, float paramFloat2, float paramFloat3, float paramFloat4)
    {
      ...
      this.mValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
      {
        ...
        {
          ItemTouchHelper.RecoverAnimation.this.setFraction(paramValueAnimator.getAnimatedFraction());
        }
      });
