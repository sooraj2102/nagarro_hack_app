  ...
  implements OpReorderer.Callback
{
  ...
  private void dispatchAndUpdateViewHolders(UpdateOp paramUpdateOp)
  {
    ...
    label290: Object localObject = paramUpdateOp.payload;
    recycleUpdateOp(paramUpdateOp);
    if (j > 0)
    {
    ...
    if (i >= 0)
    {
      ...
      if (localUpdateOp2.cmd == 8)
        if (localUpdateOp2.positionStart < localUpdateOp2.itemCount)
        {
          ...
          label64: if ((paramInt1 < k) || (paramInt1 > m))
            break label192;
      ...
      while (true)
      {
        ...
        label192: if (paramInt1 >= localUpdateOp2.positionStart)
          continue;
        if (paramInt2 == 1)
        {
          localUpdateOp2.positionStart = (1 + localUpdateOp2.positionStart);
      ...
      do
        while (true)
        {
          ...
          if (this.cmd != localUpdateOp.cmd)
            return false;
          if ((this.cmd == 8) && (Math.abs(this.itemCount - this.positionStart) == 1) && (this.itemCount == localUpdateOp.positionStart) && (this.positionStart == localUpdateOp.itemCount))
            continue;
          ...
          if (this.payload == null)
            break;
          if (!this.payload.equals(localUpdateOp.payload))
            return false;
        }
