  ...
  implements HttpCodec
{
    ...
    throws IOException
  {
    ...
    try
    {
      ...
      Response.Builder localBuilder = new Response.Builder().protocol(localStatusLine.protocol).code(localStatusLine.code).message(localStatusLine.message).headers(readHeaders());
    ...
    {
      localIOException = new IOException("unexpected end of stream on " + this.streamAllocation);
    ...
    implements Source
  {
      ...
      throws IOException
    {
      ...
      {
        localStreamAllocation.streamFinished(bool, Http1Codec.this);
        return;
      }
    }
  ...
  private class ChunkedSource extends Http1Codec.AbstractSource
  {
      ...
      throws IOException
    {
      ...
      long l = Http1Codec.this.source.read(paramBuffer, Math.min(paramLong, this.bytesRemainingInChunk));
      if (l == -1L)
      {
        endOfInput(false);
