...
public final class FormBody extends RequestBody
{
  ...
  private long writeOrCountBytes(BufferedSink paramBufferedSink, boolean paramBoolean)
  {
    long l = 0L;
    if (paramBoolean);
    for (Buffer localBuffer = new Buffer(); ; localBuffer = paramBufferedSink.buffer())
    {
      int i = 0;
      ...
      {
        if (i > 0)
          localBuffer.writeByte(38);
        localBuffer.writeUtf8((String)this.encodedNames.get(i));
        localBuffer.writeByte(61);
        localBuffer.writeUtf8((String)this.encodedValues.get(i));
        i++;
      }
    }
    if (paramBoolean)
