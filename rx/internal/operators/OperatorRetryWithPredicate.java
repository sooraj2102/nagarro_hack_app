  ...
  implements Observable.Operator<T, Observable<T>>
{
  ...
  static final class SourceSubscriber<T> extends Subscriber<Observable<T>>
  {
    ...
    {
      this.inner.schedule(new Action0(paramObservable)
      {
        public void call()
        {
          OperatorRetryWithPredicate.SourceSubscriber.this.attempts.incrementAndGet();
          1 local1 = new Subscriber(this)
          {
            boolean done;
            ...
            public void onError(Throwable paramThrowable)
            {
              ...
              {
                this.done = true;
                if ((((Boolean)OperatorRetryWithPredicate.SourceSubscriber.this.predicate.call(Integer.valueOf(OperatorRetryWithPredicate.SourceSubscriber.this.attempts.get()), paramThrowable)).booleanValue()) && (!OperatorRetryWithPredicate.SourceSubscriber.this.inner.isUnsubscribed()))
                  OperatorRetryWithPredicate.SourceSubscriber.this.inner.schedule(this.val$_self);
              }
              else
              {
                return;
              }
              OperatorRetryWithPredicate.SourceSubscriber.this.child.onError(paramThrowable);
            }
            ...
            public void onNext(T paramT)
            {
              if (!this.done)
              {
                OperatorRetryWithPredicate.SourceSubscriber.this.child.onNext(paramT);
            ...
            {
              OperatorRetryWithPredicate.SourceSubscriber.this.pa.setProducer(paramProducer);
            }
          };
